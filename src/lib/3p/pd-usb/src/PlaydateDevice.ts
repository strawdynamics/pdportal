...
import { assert, parseKeyVal, stringToBytes, bytesPos } from './utils'
...
export const USB_FILTER = {
	usbVendorId: PLAYDATE_VID,
	usbProductId: PLAYDATE_PID
}
 ...
 */
export type PlaydateDeviceEventCallback<T extends any[] = any[]> = (
	...args: T
) => void
...
export class PlaydateDevice {
	...
	on(
		eventType: 'data',
		callback: PlaydateDeviceEventCallback<[Uint8Array]>
	): void
	...
	on(
		eventType: PlaydateDeviceEvent,
		callback: PlaydateDeviceEventCallback
	): void
	on(
		eventType: PlaydateDeviceEvent,
		callback: PlaydateDeviceEventCallback
	...
	async open() {
		...
		assert(
			Array.isArray(lines),
			'Open error - Playdate did not respond, maybe something else is interacting with it?'
		)
		...
		assert(
			resp === '' || resp.includes('echo off'),
			`Open error - invalid echo command response, got ${resp}`
		)
		this.emit('open')
	}
	...
	async close() {
		...
		if (this.isPollingControls) await this.stopPollingControls()
	...
	async getVersion(): Promise<PDVersion> {
		...
		lines.forEach((line) => {
			...
			if (Array.isArray(kv)) parsed[kv[0]] = kv[1]
		})
	...
	async getScreen() {
		...
		assert(
			bytes.byteLength >= 12011,
			`Screen command response is too short, got ${bytes.byteLength} bytes`
		)
	...
	async sendBitmap(bitmap: Uint8Array) {
		...
		assert(
			bitmap.length === 12000,
			`Bitmap size is incorrect; should be 12000 (400 * 240 / 8), got ${bitmap.length}`
		)
	...
	async sendBitmapIndexed(indexed: Uint8Array) {
		assert(
			indexed.length === 96000,
			`Bitmap size is incorrect; should be 96000 (400 * 240), got ${indexed.length}`
		)
		...
		while (dstPtr < bitmapSize) {
			...
			for (let shift = 0; shift < 8; shift++) {
				...
				if (indexed[srcPtr++] !== 0) chunk |= 0x80 >> shift
			}
			bitmap[dstPtr++] = chunk
		}
	...
	getControls() {
		assert(
			this.isPollingControls,
			'Please begin polling Playdate controls by calling startPollingControls() first'
		)
	...
	buttonIsPressed(button: PDButton | PDButtonName) {
		...
		if (typeof button === 'string') button = PDButtonBitmaskMap[button]
	...
	async run(path: string) {
		assert(
			path.startsWith('/'),
			'Path must begin with a forward slash, e.g. "/System/Crayons.pdx"'
		)
	...
	private async pollControlsLoop() {
		while (this.isPollingControls && this.port.readable) {
			try {
				...
				if (this.isPollingControls) throw e
			}
		}
	...
	private assertNotBusy() {
		assert(
			!this.isBusy,
			'Device is currently busy, stop polling controls or streaming to send further commands'
		)
	}

	private parseControlState(state: string): PDControlState {
		const parsed = state.match(
			/buttons:([A-F0-9]{2}) ([A-F0-9]{2}) ([A-F0-9]{2}) crank:(\d+\.?\d+) docked:(\d)/
		)
		if (parsed) {
	...
	private parseButtonFlags(flags: number): PDButtonState {
		return {
			[PDButtonNameMap[PDButton.kButtonRight]]:
				(flags & PDButton.kButtonRight) !== 0,
			[PDButtonNameMap[PDButton.kButtonLeft]]:
				(flags & PDButton.kButtonLeft) !== 0,
			[PDButtonNameMap[PDButton.kButtonUp]]: (flags & PDButton.kButtonUp) !== 0,
			[PDButtonNameMap[PDButton.kButtonDown]]:
				(flags & PDButton.kButtonDown) !== 0,
			[PDButtonNameMap[PDButton.kButtonB]]: (flags & PDButton.kButtonB) !== 0,
			[PDButtonNameMap[PDButton.kButtonA]]: (flags & PDButton.kButtonA) !== 0,
			[PDButtonNameMap[PDButton.kButtonMenu]]:
				(flags & PDButton.kButtonMenu) !== 0,
			[PDButtonNameMap[PDButton.kButtonLock]]:
				(flags & PDButton.kButtonLock) !== 0
		} as PDButtonState
	}
}
